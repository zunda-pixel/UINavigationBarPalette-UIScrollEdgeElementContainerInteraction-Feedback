...
撮ったものです。表内は GIF（400px / 50fps）で、原寸の mp4 は各 Level の下にリンクして
あります。mp4 は 60fps です。

GIF はフレーム遅延を 1/100 秒の整数でしか持てないため、50fps（遅延 2cs）がコマ数の上限です。
滑らかさを優先する場合は mp4 をご覧ください。
ファイルはすべて `Docs/` にあります。
...
#### Level 3 — 指を離さない 1 回のドラッグ。上へ送って通常タイトルに切り替え、そのまま下へ折り返して指を離す

| 公開 API 版 | palette 版 |
|---|---|
| ![公開 API 版](Docs/lv3-public.gif) | ![palette 版](Docs/lv3-palette.gif) |

折り返して上端を越えていく**途中**を見てください。公開 API 版はラージタイトルがピッカーに
覆われて薄く消えていきますが、palette 版は覆われずくっきり出たままです。カスタムバーが
ナビゲーションバーの外にあるため、バーが伸びてもその上に残り続けるためです。

引き伸ばしきった最深部では両者とも同じ位置に落ち着くので、差が出るのは遷移の最中です。
そのあと指を離すと、システムのアニメーションで上端まで戻ります。

Level 3 の GIF は 2 倍速です。原寸の mp4 は実速度です。
...
指での操作ではありません。タッチ駆動のタイミングに依存する挙動は再現しきれない場合があります
（iOS 27 のバー最小化は、プログラムからのスクロールでは発火しませんでした）。
