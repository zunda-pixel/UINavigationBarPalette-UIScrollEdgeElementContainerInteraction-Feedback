...
#### Level 3 — ラージタイトルの位置の動きと、通常タイトルへの切り替わり
...
指での操作ではありません。タッチ駆動のタイミングに依存する挙動は再現しきれません。

実際に確認できた範囲では、上端を越えて引き伸ばした状態（負の `contentOffset` を
プログラムから設定した場合）はラージタイトルがピッカーの下に潜り込み、しかも両実装で
同じように起きました。指でのドラッグとはレイアウト経路が異なるためと考えられるため、
Level 3 の録画からは引き伸ばし区間を外してあります。引き伸ばし時の挙動を比べる場合は、
プレビューを手で操作して確認してください。
